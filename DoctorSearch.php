...
$result = mysql_query("USER.* FROM USER.HEALTHCARE_SYSTEM_USER AS USER, DOCTOR AS D, DOCTORS_HAVE AS H
            WHERE (`USER.firstname`='$query') OR (`USER.lastname`='$query') OR (`USER.ucredentials`='$query') AND USER.ucredentials=H.ucredentials AND H.dcredentials =D.dcredentials AND D.dcredentials='$doctor'") or die(mysql_error());
			
// search for user's first name, last name, or ID, and check that the signed-in doctor has that patient assigned to them
